...
return (
    <div className="bg-white/10 backdrop-blur-md p-6 rounded-2xl shadow-lg border border-white/20">
        ...
        <div className="flex items-center">
            <button onClick={togglePlayPause} className="text-white p-2 rounded-full bg-[#e28b07] hover:bg-[#c87a06] transition-colors duration-200 shadow-md">
            ...
            <div className="ml-4 w-full">
                ...
                <div className="mt-2 flex items-center">
                    <input type="range" min="0" max={duration} value={currentTime} onChange={handleSeek} className="w-full h-2 bg-gray-500/50 rounded-lg appearance-none cursor-pointer accent-[#e28b07]"/>
                ...
                <div className="space-y-3 max-h-96 overflow-y-auto">
                    {history.map((item) => (
                        <div key={item.jobId} className="w-full flex justify-between items-center p-4 bg-gray-700 rounded-lg">
                            ...
                            <button onClick={() => onPlay(item)} className="bg-[#e28b07] hover:bg-[#c87a06] text-white font-bold py-2 px-4 rounded-lg">
                                Play
                            </button>
                        </div>
                    ))}
...
function PodcastGenerator({ signOut, user }) {
  const [tokenBalance, setTokenBalance] = useState(() => {
    const savedTokenBalance = localStorage.getItem('hootpods-tokenBalance');
    return savedTokenBalance !== null ? JSON.parse(savedTokenBalance) : 0;
  });
  const [showBuyTokensModal, setShowBuyTokensModal] = useState(false);
  const [showHistoryModal, setShowHistoryModal] = useState(false);
  const [podcastHistory, setPodcastHistory] = useState(() => {
    const savedPodcastHistory = localStorage.getItem('hootpods-podcastHistory');
    return savedPodcastHistory !== null ? JSON.parse(savedPodcastHistory) : [];
  });
  ...
  const [generatedPodcast, setGeneratedPodcast] = useState(() => {
    const savedGeneratedPodcast = localStorage.getItem('hootpods-generatedPodcast');
    return savedGeneratedPodcast !== null ? JSON.parse(savedGeneratedPodcast) : null;
  });
  ...
  // --- NEW: useEffect hooks to save state to localStorage ---
  useEffect(() => {
    localStorage.setItem('hootpods-tokenBalance', JSON.stringify(tokenBalance));
  }, [tokenBalance]);

  useEffect(() => {
    localStorage.setItem('hootpods-podcastHistory', JSON.stringify(podcastHistory));
  }, [podcastHistory]);

  useEffect(() => {
    localStorage.setItem('hootpods-generatedPodcast', JSON.stringify(generatedPodcast));
  }, [generatedPodcast]);
  ...
  <div className="w-full max-w-2xl mx-auto">
    <header className="flex justify-between items-center mb-10">
        <div className="flex items-center text-3xl font-bold text-white">
            <img src="/images/IMG_4365.png" alt="HootPODS Logo" className="h-10 w-10 mr-2" />
        ...
        <div className="text-right">
            <div className="flex items-center space-x-4">
                ...
                <div className="flex items-center space-x-2 bg-gray-800 px-3 py-1.5 rounded-lg">
                    ...
                    <button onClick={() => setShowBuyTokensModal(true)} className="ml-2 bg-[#e28b07] hover:bg-[#c87a06] text-white text-xs font-bold py-1 px-2 rounded-md">+</button>
                </div>
      ...
      <div className="bg-gray-800 p-8 rounded-2xl shadow-2xl border border-gray-700">
        {isLoading && (
            <div className="text-center py-12">
                <LoaderIcon className="h-12 w-12 mx-auto text-[#e28b07]"/>
        ...
        {!isLoading && scriptPreview && (
          <div>
            ...
            <div className="flex items-center justify-between">
                ...
                <button onClick={handleApproveScript} className="px-6 py-3 font-bold text-white bg-[#e28b07] hover:bg-[#c87a06] transition-all">
        ...
        {!isLoading && !scriptPreview && (
          <form onSubmit={handleGenerateScript}>
            <div className="mb-6">
                ...
                <input type="text" id="topic" value={topic} onChange={(e) => setTopic(e.target.value)} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#e28b07]"/>
            </div>
            <div className="grid md:grid-cols-2 gap-6 mb-6">
                <div><label htmlFor="child-name" className="block mb-2 font-medium text-gray-300">Child's First Name</label><input type="text" id="child-name" value={childName} onChange={e => setChildName(e.target.value)} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#e28b07]" placeholder="e.g., Alex"/></div>
                <div>
                    ...
                    <input
                        ...
                        className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#e28b07]"
                        placeholder="e.g., 8"
                        autoComplete="off"
                    />
                </div>
                <div><label htmlFor="grade-level" className="block mb-2 font-medium text-gray-300">Grade Level</label><select id="grade-level" value={gradeLevel} onChange={e => setGradeLevel(e.target.value)} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#e28b07]">{["Kindergarten - 1st Grade", "2nd Grade - 3rd Grade", "4th Grade - 5th Grade", "6th Grade - 7th Grade", "8th Grade - 9th Grade", "10th Grade - 11th Grade", "12th Grade+"].map(o => (<option key={o} value={o}>{o}</option>))}</select></div>
                <div><label htmlFor="interests" className="block mb-2 font-medium text-gray-300">Interests</label><input type="text" id="interests" value={interests} onChange={e => setInterests(e.target.value)} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#e28b07]" placeholder="e.g., Minecraft, Dragons"/></div>
            </div>
            <div className="mb-8">
                <label htmlFor="voice" className="block mb-2 font-medium text-gray-300">Voice Style</label>
                <select id="voice" value={voice} onChange={e => setVoice(e.target.value)} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-[#e28b07]">
                    {Object.keys(voiceOptions).map(name => (<option key={name} value={name}>{name}</option>))}
                </select>
            </div>
            <button type="submit" disabled={isLoading} className="w-full flex items-center justify-center px-6 py-4 text-lg font-bold text-white bg-[#e28b07] hover:bg-[#c87a06] disabled:bg-[#a06105] disabled:cursor-not-allowed transition-all duration-300 shadow-lg">
...
<div className="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
    ...
    <div className="text-center md:text-left">
        <div className="flex items-center justify-center md:justify-start text-4xl font-bold text-white mb-4">
            <img src="/images/IMG_4365.png" alt="HootPODS Logo" className="h-12 w-12 mr-3" />
            <h1 className="ml-1">HootPODS</h1>
        </div>
